...
A `repository` allows to run queries for a specific `entity`, they can be used instead of [queriers](/docs/querying-querier). That way, it is unnecessary to provide the `entity` parameter, and only the `query` parameter is required to run the queries. Each `repository` is also associated with a single [querier](/docs/querying-querier).
